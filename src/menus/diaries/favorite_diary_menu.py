...
class FavoriteDiaryMenu(QMenu):
    def __init__(self, favoritebox, mainpage, pos : QPoint, eventpage) :
        super(FavoriteDiaryMenu, self).__init__(parent = favoritebox)
        ...
        if action == self.remove_favorite_action:
            ...
            msg.setText(f"{translate_dic[self.ui.current_lang][3]} « {item_text} » {translate_dic[self.ui.current_lang][4]}")
        ...
        # supression d'un agenda via la liste des favoris
            ...
            if msg.clickedButton() == btn_oui:
                self.ui.myagenda_box.removeItem(self.ui.myagenda_box.findText(
                    item_text))  # suppression des agenda en récupérant son indice à l'aide de son nom
